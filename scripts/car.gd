...
func _ready() -> void:
	...
	_cam.near = 0.1
	_cam.far = 1200.0
	_cam.position = Vector3(0.0, 2.4, -6.2)
	_cam.current = false
	add_child(_cam)
...
func interact() -> void:
	if world and world.has_method("_enter_car"):
		world._enter_car(self)
...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("interact") \
			...
			and world and world.has_method("_exit_car") \
			and not net_driven:
		world._exit_car(self)
		return
	...
	_cam.look_at(global_position + dir * 4.0 + Vector3(0.0, 1.2, 0.0), Vector3.UP)
...
func _ghost_physics(delta: float) -> void:
	...
	if _is_local_driver():
		...
		_fov = lerpf(_fov, 72.0 + boost * 18.0, clampf(delta * 6.0, 0.0, 1.0))
		_cam.fov = _fov
		_cam.current = true
		var dir := transform.basis.z
		_cam.look_at(global_position + dir * 4.0 + Vector3(0.0, 1.2, 0.0), Vector3.UP)
...
func _fill_audio() -> void:
	...
	var base := 44.0 + rpm_n * 130.0
	var eframes := epb.get_frames_available()
	for i in eframes:
		_e_phase += step * base
		var s := sin(_e_phase) * 0.34 + sin(_e_phase * 2.0) * 0.12 + sin(_e_phase * 0.5) * 0.2
		_noise_sm = _noise_sm * 0.92 + randf_range(-1.0, 1.0) * 0.08
		s += _noise_sm * (0.03 + 0.06 * rpm_n)
		s *= 0.8
		epb.push_frame(Vector2(s, s))
	var tframes := tpb.get_frames_available()
	var whine_f := 650.0 + rpm_n * 450.0 + boost * 1500.0
	var whine_amp := 0.04 + boost * 0.22
	var spool_noise := _spool * 0.25
	var blow := _blowoff
	for i in tframes:
		_t_phase += step * whine_f
		var s := sin(_t_phase) * whine_amp
		_noise_sm2 = _noise_sm2 * 0.78 + randf_range(-1.0, 1.0) * 0.22
		s += _noise_sm2 * spool_noise
		if blow > 0.01:
			var bfreq := 1600.0 - blow * 1300.0
			_b_phase += step * bfreq
			s += sin(_b_phase) * 0.55 * blow
			s += randf_range(-1.0, 1.0) * 0.25 * blow
		s *= 0.7
